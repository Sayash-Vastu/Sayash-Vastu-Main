...
async function loadPerformancePanel(){
  ...
  el.innerHTML = `
    ...
    <div style="display:flex;align-items:center;gap:14px;background:#f0f6ff;border:1px solid #d6e6fb;border-radius:10px;padding:13px;margin-bottom:12px">
      <div style="font-size:30px;font-weight:700;color:#185FA5;line-height:1">${mine.score}</div>
      <div style="flex:1">
        <div style="font-size:12.5px;font-weight:600;color:#1b2437">Your score this period</div>
        <div style="font-size:11.5px;color:#6b7280;margin-top:2px">${mine.score>=75?'Strong — keep it up':mine.score>=55?'Doing okay — on-time delivery will lift this most':'Mostly driven by how many tasks were assigned and closed'}</div>
      </div>
    </div>
    ...
    <div style="border-top:1px solid #f0f1f4;margin-top:12px;padding-top:10px;font-size:11.5px;color:#6b7280;line-height:1.75">
      <b style="color:#1b2437">How this is calculated</b><br>
      <b>On-time delivery</b> — of the tasks you closed, how many were done by their end date.<br>
      <b>Avg turnaround</b> — how many days a task takes you from start to close.<br>
      <b>Attendance</b> — days present out of working days so far (Sundays and holidays excluded).<br>
      <b>Tasks completed</b> — tasks closed in this period only.<br>
      <span style="color:#9aa0aa">Score = on-time 45% · tasks completed 25% · attendance 20% · no overdue 10%</span>
    </div>`;
}
